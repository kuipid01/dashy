...
import { useCreateVariant, useDeleteProductVariant, UseFetchProductVariants, useUpdateVariant } from '@/app/(handlers)/general/general';
import { toast } from 'sonner';
import { useGetCurrentProduct } from '@/app/(handlers)/product/product';
...
const ProductVariantManager = ({ productId }: { productId: number }) => {
    ...
    const { isPending: deleting, mutateAsync: deleteVariant } = useDeleteProductVariant()
    ...
    const { isPending: updating, mutateAsync: updateVariant } = useUpdateVariant()
    const [variants, setVariants] = useState<(ProductVariant | Omit<ProductVariant, "ID">)[]>([]);

    const [showAddForm, setShowAddForm] = useState(false);
    const [editingVariant, setEditingVariant] = useState<any>(null);
    const [variantTypes, setVariantTypes] = useState(['Size', 'Color', 'Material', 'Dimensions', 'Weight']);
    ...
    const resetForm = () => {
        const emptyForm: Omit<ProductVariant, "ID"> = { Sku: '', Stock: 0, Price: 0, Weight: 0 };
    ...
    const handleSubmit = async () => {
        try {
            ...
            const hasAtLeastOneTypeValue = variantTypes.some((type) =>
    ...
    const validateStock = (stock: number): boolean => {
        ...
        const totalStockInVariants = variants.reduce((sum, variant: Omit<ProductVariant, "ID">) => sum + (variant.Stock || 0), 0);
    ...
    return (
        <div className=" max-w-full lg:max-w-8xl mx-auto p-6">
            ...
            {(showAddForm || editingVariant) && <div className="bg-white border-2 border-gray-200 rounded-lg p-6 mb-4">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex justify-end gap-2 pt-4">
                        ...
                        >
                            {isPending || updating ? <Loader2 className='animate-spin duration-300' /> : <Save size={16} />}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full table-auto">
                        <thead className="bg-gray-50">
                            <tr>
                                {variantTypes.map(type => (
                                    <th key={type} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider capitalize">
                                        {type}
                                    </th>
                                ))}
